...
class ConsultaControllerTest {
    ...
    @Test
    @DisplayName("Debería retornar http 400 cuando los datos ingresados sean inválidos")
    @WithMockUser
    ...
    @Test
    @DisplayName("Debería retornar http 200 cuando los datos ingresados sean válidos")
    @WithMockUser
    void agendarEscenario2() throws Exception {
        //given
        LocalDateTime fecha = LocalDateTime.now().plusHours(1);
        Especialidad especialidad = Especialidad.CARDIOLOGIA;
        DatosDetalleConsulta datosDetalleConsulta = new DatosDetalleConsulta(null, 2L, 5L, fecha);
        ...
        MockHttpServletResponse response = mvc.perform(post("/consultas")
                ...
                .content(agendarConsultaJacksonTester
                        .write(new DatosAgendarConsulta(2L, 5L, fecha, especialidad))
                        .getJson())
        ).andReturn().getResponse();
        // then
        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());

        String jsonEsperado = detalleConsultaJacksonTester
                .write(datosDetalleConsulta)
                .getJson();
